...
def similaridade(a, b):
    ...
    inter = len(p1.intersection(p2))
    overlap = inter / min(len(p1), len(p2))
    jaccard = inter / len(p1.union(p2))
    
    # Mistura o overlap (que favorece que a frase-alvo esteja contida na do usuário) 
    # com jaccard (para valorizar quando a frase do usuário for exatamente a frase-alvo)
        ...
        if "keywords" in item:
            ...
            for kw in item["keywords"]:
                if re.search(r'\b' + re.escape(kw) + r'\b', msg.lower()):
                    qt_kw += 1
                    peso_kws += len(kw) * 0.05 # Palavras maiores (ex: 'hamburguer') dão mais pontos que curtas ('comer')
            if qt_kw > 0:
                score_kw = 0.5 + (0.1 * qt_kw) + peso_kws
